...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'route-planner', 'content-contributor', 'support-specialist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<TrekkerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/trekkers` : null),
    () => getTrekkers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Trekker',
      path: isTenantUser ? `/trekkers/view/${data?.[0]?.id}` : '/trekkers',
      entity: 'trekker',
    ...
    { name: 'Route', path: '/routes', entity: 'route', service: AccessServiceEnum.PROJECT, icon: FiBox },
